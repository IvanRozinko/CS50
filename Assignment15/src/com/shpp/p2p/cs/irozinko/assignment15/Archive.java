...
class Archive {
     ...
     */
      private void archive(String input, String output) {
        ...
        HashMap<Byte, Integer> charsUnique = getUniqueChars(input);
        //using to write bytes and nodes of binary tree
        HashMap<Byte, Node> charTable = makeTree(charsUnique);
     ...
     */
    private void archiveTable(BufferedOutputStream bufferWriter, HashMap<Byte,
        ...
        for (Map.Entry<Byte, Integer> entrySet : charsUnique.entrySet()) {
            bufferWriter.write(entrySet.getKey());
            bufferWriter.write(intToByteArray(entrySet.getValue()));
        }
    }
     ...
     */
    private int getCharTableSize(HashMap<Byte, Integer> charTable) {
     ...
     */
    private HashMap<Byte, Node> makeTree(HashMap<Byte, Integer> charsUnique) {
        HashMap<Byte, Node> charTable = new HashMap<>();
        NodesCompare comparator = new NodesCompare();
        PriorityQueue<Node> tree = new PriorityQueue<>(comparator);
        for (Map.Entry<Byte, Integer> entry : charsUnique.entrySet()){
            TreeLeaf leaf = new TreeLeaf(entry.getKey(), entry.getValue());
            charTable.put(entry.getKey(), leaf);
            tree.add(leaf);
        }
     ...
     */
    private void archiveData(HashMap<Byte, Node> charTable, BufferedOutputStream bufferWriter,
        ...
        for (int i = 0; i < dataLength; i++) {
            ...
            StringBuilder bitSequence = new StringBuilder(charTable.get(current).bitSequence);
     ...
     */
    private HashMap<Byte, Integer> getUniqueChars(String input) {
        HashMap<Byte, Integer> chars = new HashMap<>();
