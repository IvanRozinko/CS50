...
class Unarchive {
    ...
    private void unarchive(String input, String output) {
        ...
        try(FileInputStream fis = new FileInputStream(input); FileOutputStream fos = new FileOutputStream(output);
            ...
            HashMap<Byte, Integer> charsUnique = makeCharTable(bufferReader, sizeOfTable);
     ...
     */
    private void extractData(BufferedInputStream bufferReader, HashMap<Byte, Node> charTable,
                                    BufferedOutputStream bufferWriter) throws IOException {
        HashMap<String, Byte> chars = new HashMap<>();
        for (Map.Entry<Byte, Node> entry : charTable.entrySet()){
            chars.put(entry.getValue().bitSequence, entry.getKey());
        }
        ...
        for (int i = 0; i < dataLength; i++ ){
            ...
            for (int j = 0; j < sb.length(); j++){
                ...
                if(chars.containsKey(bitSequence.toString())) {
                    bufferWriter.write(chars.get(bitSequence.toString()));
     ...
     */
    private HashMap<Byte, Integer> makeCharTable(BufferedInputStream bufferReader, int sizeOfTable) throws IOException {
        int tableBlockSize = 5;
        int bytesInInteger = 4;
        HashMap<Byte, Integer> charTable = new HashMap<>();
         while (sizeOfTable > 0){
